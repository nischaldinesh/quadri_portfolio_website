...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/70 shadow-sm overflow-x-clip">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between">
              ...
              return (
                <li key={item.href}>
                  <Link
                    ...
                    className={[
                      "relative inline-flex items-center font-sans px-12 text-sm sm:text-base font-bold transition",
                      active
                        ? "text-[#841617]"
        ...
        {/* Mobile menu button (only on small) */}
        <button
          type="button"
      ...
      {/* Backdrop + Sheet only render on small screens AND only when open */}
      {open && (
        <>
          {/* Backdrop (small screens only) */}
          <div
            className="sm:hidden fixed inset-0 z-40 bg-black/40 backdrop-blur-[1px] opacity-100 transition-opacity duration-200"
            onClick={() => setOpen(false)}
          />

          {/* Slide-over (small screens only) */}
          <aside
            id="mobile-sheet"
            role="dialog"
            aria-modal="true"
            className="sm:hidden fixed right-0 top-0 z-50 h-screen w-80 max-w-[85%] bg-white shadow-2xl transform transition-transform duration-300 ease-out will-change-transform translate-x-0"
          >
            {/* sheet header */}
